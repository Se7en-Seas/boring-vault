...
import {BTCNFullMinterDecoderAndSanitizer} from
...
contract BTCNMinterIntegrationTest is Test, MerkleTreeHelper {
    ...
    function testBTCNMinter() public {
        ...
        address cornSwap = getAddress(sourceChain, "cornSwapFacility"); 
       ...
       vm.startPrank(operator);  
        ...
        _addBTCNLeafs(
            ...
            getAddress(sourceChain, "cornSwapFacility")
        );
        ...
        targets[2] = getAddress(sourceChain, "cornSwapFacility"); 
        targets[3] = getAddress(sourceChain, "cornSwapFacility"); 
        
        bytes[] memory targetData = new bytes[](4);
        targetData[0] =
            abi.encodeWithSignature("approve(address,uint256)", getAddress(sourceChain, "cornSwapFacility"), 10000e8);
        targetData[1] =
            abi.encodeWithSignature("approve(address,uint256)", getAddress(sourceChain, "cornSwapFacility"), 10000e8);
