...
contract CCIPBridgeIntegrationTest is Test, MainnetAddresses {
    ...
    function testBridgingToArbitrumERC20() external {
        deal(address(WETH), address(boringVault), 101e18);

        ManageLeaf[] memory leafs = new ManageLeaf[](2);
        leafs[0] = ManageLeaf(address(WETH), false, "approve(address,uint256)", new address[](1));
        leafs[0].argumentAddresses[0] = ccipRouter;
        leafs[1] = ManageLeaf(
            ccipRouter, false, "ccipSend(uint64,(bytes,bytes,(address,uint256)[],address,bytes))", new address[](4)
        );
        leafs[1].argumentAddresses[0] = address(uint160(arbitrumDestinationChainId));
        leafs[1].argumentAddresses[1] = address(boringVault);
        leafs[1].argumentAddresses[2] = address(WETH);
        leafs[1].argumentAddresses[3] = address(WETH);
        ...
        targets[1] = ccipRouter;

        bytes[] memory targetData = new bytes[](2);
        targetData[0] = abi.encodeWithSignature("approve(address,uint256)", ccipRouter, type(uint256).max);
        ...
        message.tokenAmounts[0].token = address(WETH);
        message.tokenAmounts[0].amount = 100e18;
        message.feeToken = address(WETH);
        message.extraArgs = abi.encode(bytes4(0x97a657c9), 0);

        targetData[1] = abi.encodeWithSignature(
            "ccipSend(uint64,(bytes,bytes,(address,uint256)[],address,bytes))", arbitrumDestinationChainId, message
        );
