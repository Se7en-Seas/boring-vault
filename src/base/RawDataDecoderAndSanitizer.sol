...
contract RawDataDecoderAndSanitizer {
    ...
    function approve(address a, uint256) external view returns (address[] memory addressesFound) {
        addressesFound = new address[](1);
        addressesFound[0] = a;
    }

    function transfer(address a, uint256) external view returns (address[] memory addressesFound) {
        addressesFound = new address[](1);
        addressesFound[0] = a;
    }
